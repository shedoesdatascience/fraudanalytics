...
# PROJECT: 20180601
#
# MODULE: 021   TEXT ANALYTICS COMPANY PROFILE
#
# DESCRIPTION:
...
# 2. clean company profile text data
...
fraudulent_profile <- txt_data_ads[ which(txt_data_ads$fraudulent==1),]
verified_profile<-txt_data_ads[ which(txt_data_ads$fraudulent==0),]
...
jd_text<-cleanFun(fraudulent_profile$company_profile)
jd_text<-gsub("[\r\n]", "", jd_text)
...
#non-fraudulent data cleaning of company profile

n_jd_text<-cleanFun(verified_profile$company_profile)
n_jd_text<-gsub("[\r\n]", "", n_jd_text)
...
#3. Perform sentiment analysis ####
#3.1 fraudulent company profile analysis
...
#4. Word clouds based on frequency of words ####
...
#3.2 non-fraudulent company profile analysis
