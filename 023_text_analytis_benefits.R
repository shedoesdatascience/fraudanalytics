...
# PROJECT: 20180601
#
# MODULE: 023 - ANALYSE - BAG OF WORDS MODELLING
#
# DESCRIPTION:
...
fraudulent_benefits <- txt_data_ads[ which(txt_data_ads$fraudulent==1),]
verified_benefits<-txt_data_ads[ which(txt_data_ads$fraudulent==0),]
...
jd_text<-cleanFun(fraudulent_benefits$benefits)
jd_text<-gsub("[\r\n]", "", jd_text)
...
#non-fraudulent data cleaning of job benefits

n_jd_text<-cleanFun(verified_benefits$benefits)
n_jd_text<-gsub("[\r\n]", "", n_jd_text)
