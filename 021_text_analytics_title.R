...
#2. clean job description text data

#subset data by whether or not the job ad is fraudulent
fraudulent_job_desc <- txt_data_ads[ which(txt_data_ads$fraudulent==1),]
verified_job_desc<-txt_data_ads[ which(txt_data_ads$fraudulent==0),]
...
jd_text<-cleanFun(fraudulent_job_desc$description)
jd_text<-gsub("[\r\n]", "", jd_text)
...
#non-fraudulent data cleaning of job description

n_jd_text<-cleanFun(verified_job_desc$description)
n_jd_text<-gsub("[\r\n]", "", n_jd_text)
...
#3.1 fraudulent job description analysis
...
#3.2 non-fraudulent job description analysis
